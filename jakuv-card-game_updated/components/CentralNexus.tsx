...
const TurnPhaseIndicator: React.FC<{ gameState: GameState }> = ({ gameState }) => {
    ...
    const thinkingText = phase === GamePhase.AI_THINKING && !gameState.actionContext ? 'AI is Thinking...' : null;
            ...
            <div className="absolute inset-0 flex items-center justify-center">
                {topThree.length > 0 ? (
                    topThree.map((card, index) => {
                        ...
                        return (
                            <div key={card.id} className="absolute transition-transform duration-200" style={{ transform: `rotate(${rotation}deg) translateY(-${(topThree.length - 1 - index) * 4}px)`, zIndex: 10 + index }}>
                                <CardComponent
                    ...
                    <div className="flex flex-col items-center justify-center text-[var(--text-dark)]">
...
const CentralNexus: React.FC<CentralNexusProps> = ({ gameState, player, onCardClick, onCardHover, onDrawCard }) => {
    const { players, currentPlayerIndex, deck, swapBar, discardPile, swapUsedThisTurn, selectedCardId, actionState, actionContext, phase, turn, counterStack } = gameState;
    ...
    const isPlayer1FirstTurn = turn === 1 && isPlayerTurn;
    const canDraw = isPlayerTurn && !swapUsedThisTurn && phase === GamePhase.PLAYER_TURN && ![ActionState.AWAITING_SCUTTLE_TARGET, ActionState.AWAITING_SURPRISE_SWAP_TARGET, ActionState.AWAITING_JACK_PLACEMENT].includes(actionState) && !isPlayer1FirstTurn;
    ...
    if (actionContext) {
        ...
        const reversedStack = [...allStackCards].reverse();
        
        const initiatorPlayerId = actionContext.playerId;
        const responderPlayerId = players.find(p => p.id !== initiatorPlayerId)!.id;

        return (
            <div className="w-full flex flex-col justify-center items-center gap-2 p-2 rounded-lg central-nexus min-h-[300px]">
                <h3 className="text-lg font-bold text-center text-[var(--warning-glow)] uppercase tracking-wider flex items-center gap-2">
                   {ICONS[actionContext.type]}
                   <span>Action Stack: {actionContext.counterVerb}</span>
                </h3>
                <div className="action-stack custom-scrollbar overflow-y-auto w-full px-20">
                    {reversedStack.map((card, index) => {
                        const stackIndex = allStackCards.length - 1 - index;
                        const isInitiator = stackIndex === 0;
                        const cardOwnerId = stackIndex % 2 === 0 ? initiatorPlayerId : responderPlayerId;
                        const cardOwner = players.find(p => p.id === cardOwnerId);

                        return (
                            <div key={card.id || index} className="action-stack-item animate-card-place">
                                <span className="action-stack-label" style={{ color: cardOwner?.isAI ? 'var(--opponent-glow)' : 'var(--player-glow)' }}>
                                    {isInitiator ? 'Initiating' : `Counter`}
                                </span>
                                <div className="card-container">
                                    <CardComponent card={{...card, isFaceUp: true}} onHover={onCardHover} />
                                </div>
                            </div>
                        );
                    })}
    ...
    const showTurnIndicator = phase !== GamePhase.START_SCREEN && phase !== GamePhase.CAMPAIGN_SELECTION && phase !== GamePhase.TUTORIAL;
