...
const WinnerModal: React.FC<WinnerModalProps> = ({ winner, winReason, onResetGame }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-80 backdrop-blur-sm flex items-center justify-center z-60 animate-fade-in">
      <div className="glassmorphic p-8 md:p-12 rounded-2xl text-center shadow-2xl border-2 w-full max-w-2xl mx-4" style={{ borderColor: winnerColor, boxShadow: `0 0 30px -5px ${winnerColor}`}}>
        ...
        <button
          onClick={onResetGame}
          className="w-full max-w-xs mx-auto text-black font-bold py-3 px-8 rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-2xl text-xl relative overflow-hidden group"
          style={{ 
