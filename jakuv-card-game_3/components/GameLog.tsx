...
import { PlayCardIcon, ScuttleIcon, DrawCardIcon, EffectIcon, SwapIcon, SystemIcon, WinnerTrophyIcon, InfoIcon, CounterIcon, WarningIcon, LockIcon, UnlockIcon, SettingsIcon, UserIcon, RestartIcon, HomeIcon, TrophyIcon } from './icons';
...
const GameLog: React.FC<GameLogProps> = ({ messages, onToggleVisibility, onResetGame, onMainMenu, onOpenSettings, onOpenProfile, onOpenAchievements, playerNames }) => {
    ...
    <div className="glassmorphic p-4 rounded-xl flex flex-col min-h-0 h-[45%] flex-shrink-0 bg-[rgba(16,8,40,0.65)]">
      ...
      <div className="mt-3 -mx-4 -mb-4 p-3 border-t-2 border-[var(--neutral-glow)]/20 bg-black/30 rounded-b-xl flex-shrink-0">
          <div className="flex items-center justify-end gap-3">
              ...
              <MicroMenuButton title="Achievements" onClick={onOpenAchievements}><TrophyIcon className="w-5 h-5" /></MicroMenuButton>
              <MicroMenuButton title="Player Profile" onClick={onOpenProfile}><UserIcon className="w-5 h-5" /></MicroMenuButton>
              <MicroMenuButton title="Game Settings" onClick={onOpenSettings}><SettingsIcon className="w-5 h-5" /></MicroMenuButton>
          </div>
      </div>
    </div>
  );
