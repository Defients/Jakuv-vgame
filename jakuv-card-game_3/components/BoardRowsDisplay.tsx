...
const BoardRowsDisplay: React.FC<BoardRowsDisplayProps> = ({ player, isCurrentPlayer, isOpponent, onCardClick, onCardHover, gameState }) => {
  ...
  const zoneClass = isOpponent ? 'opponent-zone' : 'player-zone';
  ...
  return (
    <div className={`p-4 w-full transition-all duration-500 relative flex flex-row gap-4 justify-center items-start ${turnBasedClasses} ${zoneClass} min-h-0`}>
      {royaltyRow}
      {scoreRow}
    </div>
  );
